...
def project_lidar_to_image(lidar_points, R, T, K, image):
    ...
    # OpenCV 컬러맵 적용 (각 픽셀당 개별적으로 처리)
    image_with_points = image.copy()
    black_image = np.zeros_like(image)
    
    for i in range(len(u)):
        # Z 값에 따라 색상 지정
        color_value = Z_normalized[i]
        # 컬러맵에서 색상을 가져옴
        color_mapped = cv2.applyColorMap(np.array([[color_value]], dtype=np.uint8), cv2.COLORMAP_JET)[0, 0]
        color = tuple(int(c) for c in color_mapped)
        
        cv2.circle(image_with_points, (u[i], v[i]), 3, color, -1)
        cv2.circle(black_image, (u[i], v[i]), 3, color, -1)
